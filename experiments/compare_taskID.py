...
parser = argparse.ArgumentParser("experiments/_compare_taskID.py", description=description)
parser.add_argument(
...
task_params.add_argument(
    "--experiment", type=str, default="permMNIST", choices=["permMNIST", "splitMNIST"]
)
...
if __name__ == "__main__":
    ...
    ids = [base_ids, [i + len(base_ids) for i in base_ids]]
    ...
    for id_list in ids:
        ...
        if args.n_seeds > 1:
            sems_list.append(
                [
                    np.sqrt(
                        np.var([ave_prec[seed][id] for seed in seed_list]) / (len(seed_list) - 1)
                    )
                    for id in id_list
                ]
            )
    ...
    figure = visual_plt.plot_bars(
        ...
        top_title=title,
        yerr=cis_list if args.n_seeds > 1 else None,
        ylim=(0, 1),
    )
