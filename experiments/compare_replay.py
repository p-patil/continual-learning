...
genmodel_params.add_argument(
    "--g-z-dim", type=int, default=100, help="# of latent variables (def=100)"
)
# - hyper-parameters
gen_params = parser.add_argument_group("Generator Hyper Parameters")
gen_params.add_argument(
    "--gen-iters", type=int, dest="g_iters", help="# batches to optimize generator (def=[iters])"
)
gen_params.add_argument(
    "--lr-gen", type=float, help="learning rate (separate) generator (default: lr)"
)
...
if __name__ == "__main__":
    ...
    budget_list = (
        budget_list_permMNIST if args.experiment == "permMNIST" else budget_list_splitMNIST
    )

    ## Add non-optional input argument that will be the same for all runs
    args.ewc = False
    args.ewc_lambda = 5000.0
    args.si = False
    args.si_c = 0.1
    args.xdg = False
    args.gating_prop = 0.0
    args.agem = False
    args.feedback = False
    ...
    for budget in budget_list:
        args.budget = budget
        EXR[budget] = {}
        EXR[budget] = collect_all(
            EXR[budget], seed_list, args, name="Replay Stored Data - budget = {}".format(budget)
        )
    ...
    if args.scenario == "class":
        ...
        for budget in budget_list:
            ...
            ICARL[budget] = collect_all(
                ICARL[budget], seed_list, args, name="iCaRL - budget = {}".format(budget)
            )
