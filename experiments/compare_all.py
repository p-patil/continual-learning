...
task_params.add_argument(
    "--scenario", type=str, default="task", choices=["task", "domain", "class"]
)
...
icarl_params.add_argument(
    "--budget", type=int, default=1000, dest="budget", help="how many exemplars can be stored?"
)
icarl_params.add_argument(
    "--herding", action="store_true", help="use herding to select exemplars (instead of random)"
)
icarl_params.add_argument(
    "--use-exemplars", action="store_true", help="use stored exemplars for classification?"
)
icarl_params.add_argument(
...
if __name__ == "__main__":
    ...
    key = "acc per task{}".format(
        " (all classes up to evaluated task)" if args.scenario == "class" else ""
    )
    base = {}
    ...
    for seed in seed_list:
        ...
        ave_I[seed] = [
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - NONE[seed][0][key]["task {}".format(i + 1)][i]
                    )
            ...
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - EWC[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - OEWC[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - SI[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - LWF[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - RP[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - RKD[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - AGEM[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            np.mean(
                [
                    (
                        OFF[seed][0][key]["task {}".format(i + 1)][i]
                        - ER[seed][0][key]["task {}".format(i + 1)][i]
                    )
                    for i in range(1, args.tasks)
                ]
            ),
            ...
            ave_I[seed].append(
                np.mean(
                    [
                        (
                            OFF[seed][0][key]["task {}".format(i + 1)][i]
                            - XDG[seed][0][key]["task {}".format(i + 1)][i]
                        )
                        for i in range(1, args.tasks)
                    ]
                )
            ...
            ave_I[seed].append(
                np.mean(
                    [
                        (
                            OFF[seed][0][key]["task {}".format(i + 1)][i]
                            - ICARL[seed][0][key]["task {}".format(i + 1)][i]
                        )
                        for i in range(1, args.tasks)
                    ]
                )
    ...
    if len(seed_list) > 1:
        sems = [
            np.sqrt(np.var([ave_BWT[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
        cis = [
            1.96 * np.sqrt(np.var([ave_BWT[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
    figure = visual_plt.plot_bar(
        means,
    ...
    if len(seed_list) > 1:
        sems_fwt = [
            np.sqrt(np.var([ave_FWT[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
        cis_fwt = [
            1.96 * np.sqrt(np.var([ave_FWT[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
    figure = visual_plt.plot_bar(
        means_fwt,
        ...
        ylabel="Average FWT",
        title=title,
        yerr=cis_fwt if len(seed_list) > 1 else None,
    )
    figure_list.append(figure)
    ...
    for i, name in enumerate(names):
        if len(seed_list) > 1:
            ...
            print("{:16s}    {:5.2f}            {:5.2f}".format(name, means[i], means_fwt[i]))
    print("#" * 49)
    ...
    if len(seed_list) > 1:
        sems = [
            np.sqrt(np.var([ave_F[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
        cis = [
            1.96 * np.sqrt(np.var([ave_F[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
    figure = visual_plt.plot_bar(
        means,
    ...
    if len(seed_list) > 1:
        sems_I = [
            np.sqrt(np.var([ave_I[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
        cis_I = [
            1.96 * np.sqrt(np.var([ave_I[seed][id] for seed in seed_list]) / (len(seed_list) - 1))
            for id in ids
        ]
    figure = visual_plt.plot_bar(
        means_I,
        names=names,
        colors=colors,
        ylabel="Average Intransigence",
        title=title,
        yerr=cis_I if len(seed_list) > 1 else None,
    )
    figure_list.append(figure)
    ...
    for i, name in enumerate(names):
        if len(seed_list) > 1:
            ...
            print("{:16s}    {:5.2f}            {:5.2f}".format(name, means[i], means_I[i]))
    print("#" * 49)
