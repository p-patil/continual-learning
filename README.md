...
A comparison of all methods included in this paper can be run with `experiments/compare_all.py` (this script includes extra
methods and reports additional metrics compared to the paper). The comparison in Appendix B can be run with
`experiments/compare_taskID.py`, and Figure C.1 can be recreated with `experiments/compare_replay.py`.
...
`experiments/compare_time.py`. This includes a comparison of the time these methods take to train (Figures 4 and 5).
