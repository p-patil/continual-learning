...
class AutoEncoder(Replayer):
    ...
    def __init__(
        ...
        self.lamda_vl = 1.0
        self.lamda_pl = (
            0.0  # --> when used as "classifier with feedback-connections", this should be set to 1.
        )

        self.average = (
            True  # --> makes that [reconL] and [variatL] are both divided by number of input-pixels
        )
        ...
        self.fromZ = fc_layer(
            z_dim, mlp_output_size, batch_norm=(out_nl and fc_bn), nl=fc_nl if out_nl else "none"
        )
        # -fully connected hidden layers
        self.fcD = MLP(
            input_size=fc_units,
            output_size=image_channels * image_size ** 2,
            layers=fc_layers - 1,
            hid_size=fc_units,
            drop=fc_drop,
            batch_norm=fc_bn,
    ...
    def name(self):
        ...
        hid_label = (
            "{}{}-".format("i", self.image_channels * self.image_size ** 2)
            if self.fc_layers == 1
            else ""
        )
    ...
    def calculate_recon_loss(self, x, x_recon, average=False):
        ...
        reconL = F.binary_cross_entropy(
            input=x_recon.view(batch_size, -1), target=x.view(batch_size, -1), reduction="none"
        )
    ...
    def loss_function(
        self, recon_x, x, y_hat=None, y_target=None, scores=None, mu=None, logvar=None
    ):
        ...
        reconL = self.calculate_recon_loss(
            x=x, x_recon=recon_x, average=self.average
        )  # -> possibly average over pixels
        ...
        if logvar is not None:
            ...
            if self.average:
                ...
                )  # -> divide by # of input-pixels, if [self.average]
        else:
        ...
        if scores is not None:
            ...
            distilL = utils.loss_fn_kd(
                scores=y_hat[:, :n_classes_to_consider], target_scores=scores, T=self.KD_temp
            )
        else:
    ...
    def train_a_batch(
        self, x, y, x_=None, y_=None, scores_=None, rnt=0.5, active_classes=None, task=1, **kwargs
    ):
        ...
        if x is not None:
            ...
            if active_classes is not None:
                y_hat = (
                    y_hat[:, active_classes[-1]]
                    if type(active_classes[0]) == list
                    else y_hat[:, active_classes]
                )
            # Calculate all losses
            reconL, variatL, predL, _ = self.loss_function(
                recon_x=recon_batch, x=x, y_hat=y_hat, y_target=y, mu=mu, logvar=logvar
            )
        ...
        if x_ is not None:
            ...
            if not TaskIL:
                ...
                n_replays = (
                    len(y_) if (y_ is not None) else (len(scores_) if (scores_ is not None) else 1)
                )
            ...
            for replay_id in range(n_replays):
                ...
                loss_replay[replay_id] = (
                    self.lamda_rcl * reconL_r[replay_id] + self.lamda_vl * variatL_r[replay_id]
                )
