...
class Classifier(ContinualLearner, Replayer, ExemplarHandler):
    ...
    def __init__(
        ...
        self.binaryCE_distill = (
            binaryCE_distill  # -> for classes from previous tasks, use the by the previous model
        )
        ...
        self.classifier = fc_layer(
            mlp_output_size, classes, excit_buffer=True, nl="none", drop=fc_drop
        )
    ...
    def train_a_batch(
        ...
        if x_ is not None:
            ...
            for replay_id in range(n_replays):
                ...
                if (y_ is not None) and (y_[replay_id] is not None):
                    if self.binaryCE:
                        binary_targets_ = utils.to_one_hot(y_[replay_id].cpu(), y_hat.size(1)).to(
                            y_[replay_id].device
                        )
                        predL_r[replay_id] = (
        ...
        if x is not None:
            ...
            if active_classes is not None:
                class_entries = (
                    active_classes[-1] if type(active_classes[0]) == list else active_classes
                )
            ...
            if self.binaryCE:
                ...
                if self.binaryCE_distill and (scores is not None):
                    ...
                    binary_targets = torch.cat(
                        [torch.sigmoid(scores / self.KD_temp), binary_targets], dim=1
                    )
                predL = (
                    None
                    if y is None
                ...
                predL = (
                    None if y is None else F.cross_entropy(input=y_hat, target=y, reduction="mean")
                )
