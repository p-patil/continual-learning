...
def visualize_images(tensor, name, env="main", w=400, h=400, nrow=8):
    ...
    _WINDOW_CASH[name] = _vis(env).images(
        tensor.cpu().numpy(), win=_WINDOW_CASH.get(name), nrow=nrow, opts=options
    )
...
def visualize_scalars(scalars, names, title, iteration, env="main", ylabel=None):
    ...
    scalars = [
        s.cpu().numpy() if (hasattr(s, "cpu") and hasattr(s.cpu(), "numpy")) else np.array([s])
        for s in scalars
    ]
    num = len(scalars)
