...
def get_param_stamp(args, model_name, verbose=True, replay=False, replay_model_name=None):
    ...
    # -for task
    multi_n_stamp = (
        "{n}-{set}".format(n=args.tasks, set=args.scenario) if hasattr(args, "tasks") else ""
    )
    ...
    hyper_stamp = "{i_e}{num}-lr{lr}{lrg}-b{bsz}-{optim}".format(
        ...
        num=args.epochs if args.iters is None else args.iters,
        lr=args.lr,
    ...
    if hasattr(args, "ewc") and ((args.ewc_lambda > 0 and args.ewc) or (args.si_c > 0 and args.si)):
        ewc_stamp = (
            "EWC{l}-{fi}{o}".format(
                l=args.ewc_lambda,
                    ...
                    "N" if args.fisher_n is None else args.fisher_n, "E" if args.emp_fi else ""
                ),
                o="-O{}".format(args.gamma) if args.online else "",
            )
            if (args.ewc_lambda > 0 and args.ewc)
            else ""
        )
        si_stamp = (
            "SI{c}-{eps}".format(c=args.si_c, eps=args.epsilon)
            if (args.si_c > 0 and args.si)
            else ""
        )
        ...
        if verbose and args.si_c > 0 and args.si:
            print(" --> SI:            " + si_stamp)
    ewc_stamp = (
        "--{}{}{}".format(ewc_stamp, both, si_stamp)
        if (
            hasattr(args, "ewc")
            and ((args.ewc_lambda > 0 and args.ewc) or (args.si_c > 0 and args.si))
        )
        else ""
    )
    ...
    # -for XdG
    xdg_stamp = ""
    if (hasattr(args, "xdg") and args.xdg) and (
        hasattr(args, "gating_prop") and args.gating_prop > 0
    ):
    ...
    # -for exemplars / iCaRL
    exemplar_stamp = ""
    if hasattr(args, "use_exemplars") and (
        args.add_exemplars or args.use_exemplars or args.replay == "exemplars"
    ):
        exemplar_opts = "b{}{}{}".format(
            args.budget, "H" if args.herding else "", "N" if args.norm_exemplars else ""
        )
        use = "{}{}".format(
            "addEx-" if args.add_exemplars else "", "useEx-" if args.use_exemplars else ""
        )
