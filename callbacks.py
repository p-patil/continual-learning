...
def _solver_loss_cb(
    log, visdom, model=None, tasks=None, iters_per_task=None, replay=False, progress_bar=True
):
    ...
    def cb(bar, iter, loss_dict, task=1):
        ...
        if progress_bar and bar is not None:
            ...
            bar.set_description(
                "  <SOLVER>   |{t_stm} training loss: {loss:.3} | training precision: {prec:.3} |".format(
                    t_stm=task_stm, loss=loss_dict["loss_total"], prec=loss_dict["precision"]
                )
            )
            bar.update(1)
        ...
        if (iteration % log == 0) and (visdom is not None):
            if tasks is None or tasks == 1:
                ...
                weight_new_task = 1.0 / task if replay else 1.0
            ...
            visual_visdom.visualize_scalars(
                ...
                names=names,
                iteration=iteration,
                title="SOLVER: loss ({})".format(visdom["graph"]),
                env=visdom["env"],
...
def _VAE_loss_cb(
    log, visdom, model, tasks=None, iters_per_task=None, replay=False, progress_bar=True
):
    ...
    def cb(bar, iter, loss_dict, task=1):
        ...
        if progress_bar and bar is not None:
            task_stm = "" if (tasks is None) else " Task: {}/{} |".format(task, tasks)
            bar.set_description(
                "  <VAE>      |{t_stm} training loss: {loss:.3} | training precision: {prec:.3} |".format(
                    t_stm=task_stm, loss=loss_dict["loss_total"], prec=loss_dict["precision"]
                )
            )
            bar.update(1)
        ...
        if (iteration % log == 0) and (visdom is not None):
            if tasks is None or tasks == 1:
                ...
                if replay:
                    plot_data += [
                        (1 - weight_new_task) * loss_dict["recon_r"],
