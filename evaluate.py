    ...
    for data, labels in data_loader:
        ...
        with torch.no_grad():
            if with_exemplars:
                ...
                scores = (
                    model(data) if (allowed_classes is None) else model(data)[:, allowed_classes]
                )
...
def precision(
    ...
    if visdom is not None:
        visual_visdom.visualize_scalars(
            precs,
            names=names,
            title="precision ({})".format(visdom["graph"]),
            iteration=iteration,
            env=visdom["env"],
        ...
        if n_tasks > 1 and summary_graph:
            visual_visdom.visualize_scalars(
                [average_precs],
                names=["ave"],
                title="ave precision ({})".format(visdom["graph"]),
                iteration=iteration,
                env=visdom["env"],
                ylabel="test precision",
            )
...
def initiate_metrics_dict(n_tasks, scenario):
    ...
    metrics_dict[
        "x_task"
    ] = []  # number of tasks so far (indicating the task on which training just finished)
    # Accuracy matrix
...
def intial_accuracy(
    ...
    for i in range(n_tasks):
        if not scenario == "class":
            precision = validate(
                ...
                allowed_classes=None
                if scenario == "domain"
                else list(range(classes_per_task * i, classes_per_task * (i + 1))),
                no_task_mask=no_task_mask,
                task=i + 1,
            )
    ...
    if not scenario == "class":
        ...
        metrics_dict[
            "initial acc per task (all classes up to evaluated task)"
        ] = precs_all_classes_upto_task
    return metrics_dict
...
def metric_statistics(
    ...
    for i in range(n_tasks):
        ...
        if scenario in ("domain", "class"):
            precision = (
                ...
                if (not with_exemplars) or (i < current_task)
                else 0.0
            )
            precs_all_classes.append(precision)
    ...
    if scenario == "task":
        average_precs = (
            sum([precs_only_classes_in_task[task_id] for task_id in range(current_task)])
            / current_task
        )
    elif scenario == "domain":
        average_precs = (
            sum([precs_all_classes[task_id] for task_id in range(current_task)]) / current_task
        )
    elif scenario == "class":
        average_precs = (
            sum([precs_all_classes_so_far[task_id] for task_id in range(current_task)])
            / current_task
        )
    ...
    for task_id in range(n_tasks):
        if scenario == "task":
            metrics_dict["acc per task"]["task {}".format(task_id + 1)].append(
                precs_only_classes_in_task[task_id]
            )
        elif scenario == "domain":
            metrics_dict["acc per task"]["task {}".format(task_id + 1)].append(
                precs_all_classes[task_id]
            )
        else:
            metrics_dict["acc per task (all classes)"]["task {}".format(task_id + 1)].append(
                precs_all_classes[task_id]
            )
            metrics_dict["acc per task (all classes up to trained task)"][
                "task {}".format(task_id + 1)
            ].append(precs_all_classes_so_far[task_id])
            metrics_dict["acc per task (all classes up to evaluated task)"][
                "task {}".format(task_id + 1)
            ].append(precs_all_classes_upto_task[task_id])
            metrics_dict["acc per task (only classes in task)"][
                "task {}".format(task_id + 1)
            ].append(precs_only_classes_in_task[task_id])
...
def show_samples(model, config, pdf=None, visdom=None, size=32, title="Generated images"):
    ...
    if visdom is not None:
        visual_visdom.visualize_images(
            tensor=image_tensor,
            name="Generated samples ({})".format(visdom["graph"]),
            env=visdom["env"],
            nrow=nrow,
        )
...
def show_reconstruction(
    model, dataset, config, pdf=None, visdom=None, size=32, task=None, collate_fn=None
):
    ...
    data_loader = utils.get_data_loader(
        dataset, size, cuda=model._is_on_cuda(), collate_fn=collate_fn
    )
