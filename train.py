...
def train_cl(
    ...
    for task, train_dataset in enumerate(train_datasets, 1):
        ...
        if scenario == "task":
            ...
            active_classes = [
                list(range(classes_per_task * i, classes_per_task * (i + 1))) for i in range(task)
            ]
        elif scenario == "class":
        ...
        for batch_index in range(1, iters_to_use + 1):
            ...
            if iters_left == 0:
                data_loader = iter(
                    utils.get_data_loader(training_dataset, batch_size, cuda=cuda, drop_last=True)
                )
            ...
            if Exact:
                if scenario == "task":
                    up_to_task = task if replay_mode == "offline" else task - 1
                    batch_size_replay = (
                        int(np.ceil(batch_size / up_to_task)) if (up_to_task > 1) else batch_size
                    )
            ...
            if replay_mode == "offline" and scenario == "task":
                ...
                y = (
                    y - classes_per_task * (task - 1) if scenario == "task" else y
                ...
                binary_distillation = (
                    hasattr(model, "binaryCE") and model.binaryCE and model.binaryCE_distill
                )
            ...
            if Exact:
                ...
                if scenario in ("domain", "class"):
                    ...
                    if model.replay_targets == "soft":
                        ...
                        scores_ = (
                            scores_[:, : (classes_per_task * (task - 1))]
                            if scenario == "class"
                            else scores_
                        )
            ...
            if Generative or Current:
                ...
                if scenario in ("domain", "class") and (
                    ...
                    scores_ = (
                        all_scores_[:, : (classes_per_task * (task - 1))]
                        if scenario == "class"
                        else all_scores_
                    )
                    ...
                    for task_id in range(task - 1):
                        ...
                        if (
                            hasattr(previous_model, "mask_dict")
        ...
        if isinstance(model, ContinualLearner) and (model.ewc_lambda > 0):
            ...
            allowed_classes = (
                ...
                if scenario == "task"
                else (list(range(classes_per_task * task)) if scenario == "class" else None)
            )
        ...
        if (add_exemplars or use_exemplars) or replay_mode == "exemplars":
            ...
            new_classes = (
                ...
                if scenario == "domain"
                else list(range(classes_per_task * (task - 1), classes_per_task * task))
            )
        ...
        if replay_mode == "generative":
            Generative = True
            previous_generator = (
                copy.deepcopy(generator).eval() if generator is not None else previous_model
            )
