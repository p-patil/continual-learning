...
class ContinualLearner(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        # XdG:
        self.mask_dict = (
            None  # -> <dict> with task-specific masks for each hidden fully-connected layer
        )
        ...
        self.epsilon = (
            0.1  # -> dampening parameter: bounds 'omega' when squared parameter-change goes to 0
        )

        # -EWC:
        self.ewc_lambda = (
            0  # -> hyperparam: how strong to weigh EWC-loss ("regularisation strength")
        )
        self.gamma = 1.0  # -> hyperparam (online EWC): decay-term for old tasks' contribution to quadratic term
        self.online = (
            True  # -> "online" (=single quadratic term) or "offline" (=quadratic term per task) EWC
        )
        self.fisher_n = (
            None  # -> sample size for estimating FI-matrix (if "None", full pass over dataset)
        )
    ...
    def apply_XdGmask(self, task):
        ...
        for i, excit_buffer in enumerate(self.excit_buffer_list):
            ...
            gating_mask[self.mask_dict[task][i]] = 0.0  # -> find task-specifc mask
        ...
        for n, p in self.named_parameters():
            if p.requires_grad:
                ...
                self.register_buffer(
                    "{}_EWC_prev_task{}".format(n, "" if self.online else self.EWC_task_count + 1),
                    p.detach().clone(),
                )
            ...
            for task in range(1, self.EWC_task_count + 1):
                for n, p in self.named_parameters():
                    if p.requires_grad:
                        ...
                        n = n.replace(".", "__")
                        mean = getattr(
                            self, "{}_EWC_prev_task{}".format(n, "" if self.online else task)
                        )
                        fisher = getattr(
                            self, "{}_EWC_estimated_fisher{}".format(n, "" if self.online else task)
                        )
